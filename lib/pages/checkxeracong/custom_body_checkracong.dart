...
class _BodyCheckRaCongScreenState extends State<BodyCheckRaCongScreen> with TickerProviderStateMixin, ChangeNotifier {
  ...
  Future<void> postData(String? soKhung, bool check) async {
    _isLoading = true;

    try {
      final http.Response response = await requestHelper.postData('Kho/CheckRaCong?SoKhung=$soKhung&Check=$check', _data?.toJson());
  ...
  _onSave(String? soKhung, bool? check) {
  ...
  Future<void> getDSXVanChuyen(String? tuNgay, String? denNgay, String? doiTac_Id, String? KhoXe_Id, String? BaiXe_Id, String? DongXe_Id, String? keyword) async {
    ...
    try {
      final http.Response response =
          await requestHelper.getData('Kho/GetDanhSachXeTrenLongAll?TuNgay=$tuNgay&DenNgay=$denNgay&DoiTac_Id=$doiTac_Id&KhoXe_Id=$KhoXe_Id&BaiXe_Id=$BaiXe_Id&DongXe_Id=$DongXe_Id&keyword=$keyword');
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
                                      ...
                                      Checkbox(
                                        ...
                                        onChanged: (bool? value) {
                                          setState(() {
                                            ...
                                            _onSave(item.soKhung, item.isCheck ?? false); // Gọi hàm lưu dữ liệu
                                          });
                                        },
                                      ),
