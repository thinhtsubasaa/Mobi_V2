...
class DSXacNhanGiaoXeHoPage extends StatefulWidget {
  const DSXacNhanGiaoXeHoPage({super.key});

  @override
  State<DSXacNhanGiaoXeHoPage> createState() => _DSXacNhanGiaoXeHoPage();
}

class _DSXacNhanGiaoXeHoPage extends State<DSXacNhanGiaoXeHoPage> with SingleTickerProviderStateMixin, ChangeNotifier {
  ...
  Future<void> getLichSuThayDoiKH() async {
    ...
    try {
      final http.Response response = await requestHelper.getData('LichSuYeuCauGiaoXeHo/LichSuThongTinYeuCauGiaoXeHo');
  ...
  Future<void> getListThayDoiKH() async {
    ...
    try {
      final http.Response response = await requestHelper.getData('LichSuYeuCauGiaoXeHo/GetThongTinYeuCauGiaoXeHo');
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
        if (decodedData != null) {
  ...
  Future<void> postData(GiaoXeHoModel scanData, String? viTri, String? trangThai, String? liDo) async {
    _isLoading = true;
    ...
    try {
      ...
      final http.Response response = await requestHelper.postData('LichSuYeuCauGiaoXeHo/XacNhanYeuCauGiaoXeHo?ViTri=$viTri&TrangThai=$trangThai&LyDo=$liDo', dataList.map((e) => e.toJson()).toList());
      ...
      if (response.statusCode == 200) {
        ...
        body = "Bạn vừa được xác nhận 1 yêu cầu giao xe hộ";
      } else {
  ...
  Future<void> postDataHuy(GiaoXeHoLSModel scanData) async {
    _isLoading = true;
    ...
    try {
      ...
      final http.Response response = await requestHelper.postData('LichSuYeuCauGiaoXeHo/HuyThayDoiXacNhanGiaoHo', newScanData.toJson());
      print("statusCode: ${response.statusCode}");
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
  ...
  Future<void> postDataList(List<GiaoXeHoModel> dataList, String? viTri, String? trangThai, String? liDo) async {
    _isLoading = true;

    try {
      final http.Response response = await requestHelper.postData('LichSuYeuCauGiaoXeHo/XacNhanYeuCauGiaoXeHo?ViTri=$viTri&TrangThai=$trangThai&LyDo=$liDo', dataList.map((item) => item.toJson()).toList());
      print("statusCode: ${response.statusCode}");
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
        print("data: ${decodedData}");
  ...
  _onSave(int index) async {
    ...
    _data?.ngayGiaoXe = item?.ngayGiaoXe;

    // _data?.ngayXuatKho = item?.ngayGiaoXe;
    _data?.ngayXuatKho = item?.ngayGiaoXe != null ? DateFormat('yyyy-MM-dd HH:mm').format(DateFormat('dd/MM/yyyy HH:mm').parse(item!.ngayGiaoXe!)) : null;
    print("ngayxuatkho = ${_data?.ngayGiaoXe}");
    if (_IsXacNhan == true) {
      _data?.trangThai = "1";
    }
    if (_IsTuChoi == true) {
  ...
  _onSaveList(List<int> selectedIndexes) async {
    ...
    List<GiaoXeHoModel> selectedItemsData = [];
    for (int index in selectedIndexes) {
      final item = _kehoachList?[index];
      if (item != null) {
        GiaoXeHoModel requestData = GiaoXeHoModel(
          id: item.id,
          soKhung: item.soKhung,
          taiXe_Id: item.taiXe_Id,
          nguoiYeuCau: item.nguoiYeuCau,
          ngayGiaoXe: item.ngayGiaoXe,
          mauXe: item.mauXe,
          diaDiem_Id: item?.diaDiem_Id,
          khoDen_Id: item?.khoDen_Id,
          ngayXuatKho: item?.ngayGiaoXe != null ? DateFormat('yyyy-MM-dd HH:mm').format(DateFormat('dd/MM/yyyy HH:mm').parse(item!.ngayGiaoXe!)) : null,
                                                      ...
                                                      children: [
                                                        InfoColumn(
                                                          ...
                                                          onNgayGiaoChanged: (newNgayGiao) {
                                                            setState(() {
                                                              item?.ngayGiaoXe = newNgayGiao; // Cập nhật ngày giao mới
                                                              print(item?.ngayGiaoXe);
                                                            });
                                                          },
                                                        ),
                                                      ],
