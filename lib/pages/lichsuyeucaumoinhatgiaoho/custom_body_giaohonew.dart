...
class CustomBodyDSDaXacNhanGiaoHoNew extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(child: BodyDSDaXacNhanGiaoHoNewScreen());
  }
}

class BodyDSDaXacNhanGiaoHoNewScreen extends StatefulWidget {
  const BodyDSDaXacNhanGiaoHoNewScreen({super.key});

  @override
  _BodyDSDaXacNhanGiaoHoNewScreenState createState() => _BodyDSDaXacNhanGiaoHoNewScreenState();
}

class _BodyDSDaXacNhanGiaoHoNewScreenState extends State<BodyDSDaXacNhanGiaoHoNewScreen> with TickerProviderStateMixin, ChangeNotifier {
  ...
  GiaoXeHoModel? _kehoachList;
  GiaoXeHoModel? get kehoachList => _kehoachList;
  ...
  Future<void> getListThayDoiKH() async {
    ...
    try {
      final http.Response response = await requestHelper.getData('LichSuYeuCauGiaoXeHo/GetLichSuYeuCauGiaoHo_New');
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
        print("data: ${decodedData}");
        if (decodedData != null) {
          _kehoachList = GiaoXeHoModel(
            id: decodedData['id'],
            soKhung: decodedData['soKhung'],
            nguoiYeuCau: decodedData['nguoiYeuCau'],
            nguoiXacNhan: decodedData['nguoiXacNhan'],
            ngayGiaoXe: decodedData['ngayGiaoXe'],
                  ...
                  child: Column(
                    ...
                    children: [
                      InfoColumn(
                        ...
                        noiGiao: _kehoachList?.noiGiao ?? "",
                        ngayGiao: _kehoachList?.ngayGiaoXe ?? "",
