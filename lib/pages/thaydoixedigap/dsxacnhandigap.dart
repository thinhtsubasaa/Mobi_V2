...
class DSXacNhanDiGapPage extends StatefulWidget {
  const DSXacNhanDiGapPage({super.key});

  @override
  State<DSXacNhanDiGapPage> createState() => _DSChoXacNhanDiGapPage();
}

class _DSChoXacNhanDiGapPage extends State<DSXacNhanDiGapPage> with SingleTickerProviderStateMixin, ChangeNotifier {
  ...
  Future<void> getLichSuThayDoiKH() async {
    ...
    try {
      final http.Response response = await requestHelper.getData('XeDiGap/LichSuThongTinYeuCauThayDoiKHDiGap');
  ...
  Future<void> postData(KeHoachGiaoXeModel scanData, String? trangThai, String? liDo) async {
    ...
    try {
      ...
      final http.Response response = await requestHelper.postData('XeDiGap/XacNhanYeuCauThayDoiKHDiGap?TrangThai=$trangThai&LyDo=$liDo', dataList.map((e) => e.toJson()).toList());
      ...
      if (response.statusCode == 200) {
        ...
        body = "Bạn vừa được xác nhận 1 yêu cầu thay đổi xe trung chuyển đi gấp";
      } else {
  ...
  Future<void> postDataList(List<KeHoachGiaoXeModel> dataList, String? trangThai, String? liDo) async {
    ...
    try {
      final http.Response response = await requestHelper.postData('XeDiGap/XacNhanYeuCauThayDoiKHDiGap?TrangThai=$trangThai&LyDo=$liDo', dataList.map((item) => item.toJson()).toList());
      print("statusCode: ${response.statusCode}");
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
        print("data: ${decodedData}");
  ...
  Future<void> postDataFireBase(KeHoachModel? scanData, String? body, String? nguoiYeuCau) async {
    ...
    try {
      ...
      final http.Response response = await requestHelper.postData('FireBase/XeDiGap?body=$body&NguoiYeuCau=$nguoiYeuCau', newScanData?.toJson());
  ...
  _onSaveList(List<int> selectedIndexes) async {
    ...
    for (int index in selectedIndexes) {
      final item = _kehoachList?[index];
      if (item != null) {
        KeHoachGiaoXeModel requestData =
            KeHoachGiaoXeModel(id: item.id, keHoachGiaoXe_Id: item.keHoachGiaoXe_Id, nguoiYeuCau: item.nguoiYeuCau, trangThai: _IsXacNhan ? "1" : (_IsTuChoi ? "2" : null), isDiGap: _IsXacNhan);
...
class InfoColumn extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      // onTap: onLongPress,
      onTap: !isThayDoi ? null : onLongPress,
