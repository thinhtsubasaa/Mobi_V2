...
import 'package:Thilogi/models/kpi/lidoKPI.dart';
...
import 'ChiTietDanhGiaKPI/DanhGia_ChitietKPI.dart';
import 'XemChiTiet/ChitietKPI.dart';

class CustomBodyDanhGiaKPI extends StatelessWidget {
  CustomBodyDanhGiaKPI();
  @override
  Widget build(BuildContext context) {
    return Container(
        child: BodyDanhGiaKPIScreen(
      lstFiles: [],
    ));
  }
}

class BodyDanhGiaKPIScreen extends StatefulWidget {
  final List<CheckSheetFileModel?> lstFiles;
  const BodyDanhGiaKPIScreen({super.key, required this.lstFiles});

  @override
  _BodyDanhGiaKPIScreenState createState() => _BodyDanhGiaKPIScreenState();
}

class _BodyDanhGiaKPIScreenState extends State<BodyDanhGiaKPIScreen> with TickerProviderStateMixin, ChangeNotifier {
  ...
  List<LyDoModel> _liDoList = [];
  bool IsCVKPIDonVi = false;
  ...
  (Color, Color) _statusColor(int? trangThaiOrNull) {
    ...
    switch (trangThaiOrNull) {
      ...
      case 1:
        return (const Color(0xFFE2F5F0), const Color(0xFF0E766E)); // Chờ bạn duyệt
      case 2:
  ...
  Future<void> checkIsCVKPIDonVi() async {
    try {
      final http.Response response = await requestHelper.getData('vptq_kpi_KPICaNhan/is-chuyen-vien-kpi-don-vi');
  ...
  Future<void> getListData(
    ...
    try {
      ...
      final http.Response response = await requestHelper.getData('vptq_kpi_DanhGiaKPICaNhan${q.isEmpty ? '' : '?$q'}');
      if (response.statusCode == 200) {
        ...
        final st = <TabFilter>[
          ...
          (() {
            ...
            return TabFilter(label: 'Chưa đánh giá', count: dl['soLuongChuaDanhGia'] ?? 0, bg: bg, fg: fg, trangThai: 5);
          })(),
          (() {
  ...
  Future<void> exportPDFPI(
    ...
    try {
      final url = 'vptq_kpi_DanhGiaKPICaNhan/export-pdf?id=$id';
      ...
      final fileName = 'KPIDanhGiaCaNhan_$doiTuong.pdf';
  ...
  Future<void> exportExcel(
    ...
    try {
      final url = 'vptq_kpi_DanhGiaKPICaNhan/export-excel?id=$id';
      final http.Response res = await requestHelper.postData(url, null);
      if (res.statusCode != 200) return;
      ...
      final bytes = base64Decode(b64);
      final fileName = 'DanhGiaKPICaNhan_$doiTuong.xlsx';
  ...
  Future<void> getLuongDuyet(
    ...
    try {
      const url = 'vptq_kpi_DanhGiaKPICaNhan/luong-duyet?vptq_kpi_KPICaNhan_Id';
  ...
  Future<void> getGhiChu(
    ...
    try {
      const url = 'vptq_kpi_KPICaNhan/ghi-chu?isDanhGia=true&vptq_kpi_KPICaNhan_Id';
  ...
  Future<void> getLyDo() async {
    try {
      const url = 'vptq_kpi_LyDoKhongDanhGia?page=-1';
      final http.Response response = await requestHelper.getData(url);
      if (response.statusCode == 200) {
        final decoded = jsonDecode(response.body);
        final list = (decoded as List).map((e) => LyDoModel.fromJson(e as Map<String, dynamic>)).toList();
        setState(() => _liDoList = list);
      } else {
        setState(() => _liDoList = []);
      }
      setState(() {
  ...
  Future<void> saveData(
    ...
    try {
      ...
      final res = await requestHelper.putData(
        'vptq_kpi_DanhGiaKPICaNhan/khong-danh-gia/$id?vptq_kpi_LyDoKhongDanhGia_Id=$lyDo_Id',
        null, // không body
      );
      ...
      if (res.statusCode == 200) {
        QuickAlert.show(
          ...
          title: 'Thành công',
          text: 'Xác nhận nhân sự không đánh giá thành công',
  ...
  void _showConfirmationDialogTuChoi(
    ...
    QuickAlert.show(
        ...
        text: 'Xác nhận nhân sự không đánh giá?',
        title: '',
  ...
  Future<void> openKhongDanhGiaDialog(String? id, String? tenUser) async {
    String? lyDo_Id;
                  ...
                  Row(
                    ...
                    children: [
                      Expanded(
                        child: Center(
                          child: Text(
                            'Xác nhận nhân sự $tenUser không đánh giá',
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: _liDoList
                        .map((e) => DropdownMenuItem<String>(
                              ...
                              value: e.id,
                              child: Text(e.lyDoKhongDanhGia ?? ""),
                            ))
                        .toList(),
                    onChanged: (v) => setSt(() => lyDo_Id = v),
                    decoration: InputDecoration(
                      labelText: 'Lý do không đánh giá *',
                      border: OutlineInputBorder(
                    ...
                    validator: (v) => v == null ? 'Lý do không đánh giá' : null,
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        ...
                        _showConfirmationDialogTuChoi(ctx, id, lyDo_Id);
                      },
                                  ...
                                  if (_danhGiaKPIList != null && _danhGiaKPIList!.isNotEmpty)
                                    ListView.separated(
                                      ...
                                      itemBuilder: (_, i) {
                                        ...
                                        final canNoRate = IsCVKPIDonVi && e.isKhongDanhGia == false;
                                        return KpiCard(
                                            item: e,
                                            onRate: canRate
                                                ? () {
                                                ...
                                                : null,
                                            menuPressed: () async {
                                              final action = await showFilterSheet(context, canRate: canRate, canNoRate: canNoRate); // <-- dùng bottom sheet ở đây
                                              ...
                                              switch (action) {
                                                case FilterAction.danhGia:
                                                  nextScreen(
                                                    context,
                                                    DanhGiaChiTietKPIPage(id: e.id, kyDanhGia: e.thoiDiem ?? ''),
                                                  );
                                                  break;
                                                case FilterAction.tuChoi:
                                                  await getLyDo();
                                                  if (!mounted) break;
                                                  await openKhongDanhGiaDialog(e.id, e.tenUser);
                                                  // ...
                                                  break;
...
enum FilterAction { danhGia, taiExcel, taiPdf, xemChiTiet, xemGhiChu, luongDuyet, tuChoi }

Future<FilterAction?> showFilterSheet(BuildContext context, {required bool canRate, required bool canNoRate}) {
          ...
          child: Column(
            ...
            children: [
              ...
              _SheetItem(
                ...
                onTap: canNoRate ? () => Navigator.pop(ctx, FilterAction.tuChoi) : null,
              ),
              const Divider(height: 1),
              _SheetItem(
...
enum KpiStatus { chuaDanhGia, dangXuLy, hoanThanh, traLai }
...
extension KpiStatusView on KpiStatus {
  String get label => switch (this) {
        KpiStatus.chuaDanhGia => 'Chưa đánh giá',
        KpiStatus.dangXuLy => 'Đang xử lý',
        KpiStatus.hoanThanh => 'Hoàn thành',
        KpiStatus.traLai => 'Trả lại',
      };

  Color get bg => switch (this) {
        KpiStatus.chuaDanhGia => const Color(0xFFEFF6FF),
        KpiStatus.dangXuLy => const Color(0xFFFFF7ED),
        KpiStatus.hoanThanh => const Color(0xFFECFDF5),
        KpiStatus.traLai => const Color(0xFFFFE4E6),
      };

  Color get fg => switch (this) {
        KpiStatus.chuaDanhGia => const Color(0xFF1D4ED8),
        KpiStatus.dangXuLy => const Color(0xFFCA8A04),
        KpiStatus.hoanThanh => const Color(0xFF047857),
        KpiStatus.traLai => const Color(0xFFDC2626),
      };
}
...
class _KpiCardState extends State<KpiCard> {
                  ...
                  children: [
                    Expanded(
                      child: FilledButton(
                        ...
                        child: const Text('ĐÁNH GIÁ', style: TextStyle(fontWeight: FontWeight.w800, letterSpacing: .3)),
                      ),
                    ),
  ...
  _Status _statusOf(DanhGiaKPIModel e) {
    ...
    switch (e.trangThai) {
      case 1:
        return const _Status('Chờ duyệt', Color(0xFFE2F5F0), Color(0xFF0E766E));
      case 2:
      ...
      case 4:
        return const _Status('Trả lại', Color(0xFFFFE4E4), Color(0xFFB91C1C));
      case 5:
        return const _Status('Chưa đánh giá', Color(0xFFDDEAFE), Color(0xFF2563EB));
      default:
        return const _Status('Không đánh giá', Color(0xFFFFE4E4), Color(0xFFB91C1C));
    }
  }
}
