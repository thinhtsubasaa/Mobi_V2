...
class CustomBodyChiTietKPI2 extends StatelessWidget {
  final String? id;
  final bool isChiTiet;
  CustomBodyChiTietKPI2({required this.id, required this.isChiTiet});
  @override
  Widget build(BuildContext context) {
    return Container(
        child: BodyChiTietKPIScreen2(
      id: id,
      lstFiles: [],
      isChiTiet: isChiTiet,
    ));
...
class BodyChiTietKPIScreen2 extends StatefulWidget {
  final String? id;
  final List<CheckSheetFileModel?> lstFiles;
  final bool isChiTiet;
  const BodyChiTietKPIScreen2({super.key, required this.id, required this.lstFiles, required this.isChiTiet});

  @override
  _BodyChiTietKPIScreen2State createState() => _BodyChiTietKPIScreen2State();
}

class _BodyChiTietKPIScreen2State extends State<BodyChiTietKPIScreen2> with TickerProviderStateMixin, ChangeNotifier {
  ...
  Future<void> BatDaXem(
    ...
    try {
      ...
      final http.Response response = await requestHelper.putData('vptq_kpi_KPICaNhan/da-xem/${widget.id}${q.isEmpty ? '' : '?$q'}', null);
...
// y chang renderNhanXet bên React
  ...
  Future<void> saveTuChoi(
    ...
    try {
      ...
      final res = await requestHelper.putData(
        'vptq_kpi_DanhGiaKPICaNhan/tra-cap-duoi/$id',
        payload, // không body
      );
      ...
      if (res.statusCode == 200) {
        QuickAlert.show(
          ...
          type: QuickAlertType.success,
          title: 'Thành công',
          text: 'Từ chối duyệt đánh giá KPI cá nhân thành công',
          confirmBtnText: 'Đồng ý',
          onConfirmBtnTap: () {
  ...
  Future<void> saveDuyet(
    ...
    try {
      ...
      final res = await requestHelper.putData(
        'vptq_kpi_DanhGiaKPICaNhan/gui-duyet-cap-tren/$id',
        null, // không body
      );
      ...
      if (res.statusCode == 200) {
        QuickAlert.show(
          ...
          type: QuickAlertType.success,
          title: 'Thành công',
          text: 'Duyệt đánh giá KPI thành công!',
          confirmBtnText: 'Đồng ý',
          onConfirmBtnTap: () {
            Navigator.pop(context);
            Navigator.pop(context);
  ...
  Widget build(BuildContext context) {
    ...
    final tongDiem = data.diemKetQuaCuoiCung ?? ((data.diemKetQuaTamThoi ?? 0) + (data.diemCong ?? 0) - (data.diemTru ?? 0));
          ...
          Padding(
            ...
            child: Column(
              children: [
                const SizedBox(height: 12),
                _TongDiemGrid(
                  diemCuoi: data.diemKetQuaCuoiCung ?? tongDiem ?? 0,
...
class _TongDiemGrid extends StatelessWidget {
  ...
  final String xepLoai; // ví dụ: data.xepLoai ?? '—'
          ...
          children: [
            ...
            SizedBox(
              ...
              child: _MiniKpiCard(
                ...
                value: xepLoai, // là text
                title: 'KQ xếp loại KPI',
                isTextValue: true,
              ),
            ),
            SizedBox(
...
class _MiniKpiCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      width: 20.w,
      height: 8.h,
      padding: const EdgeInsets.fromLTRB(14, 10, 16, 5),
      decoration: BoxDecoration(
        color: bg,
...
class _PiItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    ;
    String _fmt2(num? v) => (v ?? 0).toStringAsFixed(2);
    ct.diemKetQuaLanhDao = (ct.diemKetQuaLanhDao != null || ct.diemKetQuaLanhDao == 0) ? ct.diemKetQuaLanhDao : ct.diemKetQuaTuDanhGia;
    ct.diemTrongSoLanhDao = (ct.diemTrongSoLanhDao != null || ct.diemTrongSoLanhDao == 0) ? ct.diemTrongSoLanhDao : ct.diemTrongSoTuDanhGia;
    print("ct.diemKetQuaLanhDao:${ct.diemKetQuaLanhDao}");
    print("ct.diemTrongSoLanhDao:${ct.diemTrongSoLanhDao}");
        ...
        child: AnimatedContainer(
          ...
          child: Column(
            children: [
              // --- header (thu gọn) ---
              Padding(
                padding: const EdgeInsets.fromLTRB(10, 10, 1, 8),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // text
