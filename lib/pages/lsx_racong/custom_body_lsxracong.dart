...
class _BodyLSRaCongScreenState extends State<BodyLSRaCongScreen> with TickerProviderStateMixin, ChangeNotifier {
  ...
  Future<void> getRaCong(String? VungMien_Id) async {
    _baove = [];
    try {
      final http.Response response = await requestHelper.getData('BaoVe/GetAllTable_VungMien?VungMien_Id=$VungMien_Id');
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
        print("data: " + decodedData.toString());
        if (decodedData != null) {
  ...
  Future<void> getDSXRaCong(String? tuNgay, String? denNgay, String? CongBaoVe_Id, String? keyword) async {
    _dn = [];

    try {
      final http.Response response = await requestHelper.getData('KhoThanhPham/GetDanhSachXeRaCongAll?TuNgay=$tuNgay&DenNgay=$denNgay&CongBaoVe_Id=$CongBaoVe_Id&keyword=$keyword');
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
  ...
  void _selectDate(BuildContext context) async {
    ...
    if (picked != null && picked != DateTime.now()) {
      ...
      await getDSXRaCong(selectedFromDate, selectedToDate, congBaoVe_Id ?? "", maNhanVienController.text);
    }
  }
                                                ...
                                                onChanged: (newValue) {
                                                  ...
                                                  if (newValue != null) {
                                                    if (newValue == '') {
                                                      ...
                                                      getDSXRaCong(selectedFromDate, selectedToDate, newValue, maNhanVienController.text);
                                          ...
                                          child: Padding(
                                            ...
                                            child: TextField(
                                              ...
                                              decoration: const InputDecoration(
                                                ...
                                                isDense: true,
                                                hintText: 'Tìm số khung, biển số, tài xế',
