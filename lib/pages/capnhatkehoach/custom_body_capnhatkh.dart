...
class CustomBodyCapNhatKH extends StatelessWidget {
  final String? soKhung;
  CustomBodyCapNhatKH({this.soKhung});
  @override
  Widget build(BuildContext context) {
    return Container(
        child: BodyCapNhatKHScreen(
      soKhung: soKhung,
      lstFiles: [],
    ));
  }
...
class BodyCapNhatKHScreen extends StatefulWidget {
  final String? soKhung;
  final List<CheckSheetFileModel?> lstFiles;
  const BodyCapNhatKHScreen({super.key, required this.lstFiles, this.soKhung});

  @override
  _BodyCapNhatKHScreenState createState() => _BodyCapNhatKHScreenState();
}

class _BodyCapNhatKHScreenState extends State<BodyCapNhatKHScreen> with TickerProviderStateMixin, ChangeNotifier {
  ...
  Future<void> getListThayDoiKH() async {
    ...
    try {
      final http.Response response = await requestHelper.getData('Kho/GetListThongTinKeHoach');
  ...
  void getDiaDiem() async {
    try {
      final http.Response response = await requestHelper.getData('DM_DiaLy_DiaDiem');
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
        _diadiemList = (decodedData as List).map((item) => DiaDiemModel.fromJson(item)).toList();
  ...
  Future<void> postData(KeHoachGiaoXeModel scanData) async {
    ...
    try {
      ...
      final http.Response response = await requestHelper.postData('Kho/CapNhatKH', newScanData.toJson());
  ...
  _onScan(value) {
    ...
    _bl.getData(context, value).then((_) {
      setState(() {
        ...
        NoiDiId = _data?.noiDi_Id ?? "";
        NoiDenId = _data?.noiDen_Id ?? "";
      });
    });
  }
...
// Chuyển đổi danh sách URL thành chuỗi cách nhau bởi dấu phẩy
    ...
    _data?.id = _bl.khgx?.id;
    _data?.soKhung = _bl.khgx?.soKhung;
    _data?.taiXe_Id = TaiXeId;
    _data?.doiTac_Id = DoiTacId;
    _data?.tmS_DanhSachPhuongTien_Id = BienSoId;
    // _data?.noiDi_Id = NoiDiId;
    _data?.noiDen_Id = NoiDenId;
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton2<String>(
                        ...
                        onChanged: (newValue) {
                          ...
                          if (newValue != null) {
                            _handleBarcodeScanResult(newValue);
                          }
                        },
                        ...
                        dropdownSearchData: DropdownSearchData(
                          ...
                          searchMatchFn: (item, searchValue) {
                            if (item is DropdownMenuItem<String>) {
                              ...
                              return _kehoachList?.any((baiXe) => baiXe.soKhung == itemId && baiXe.soKhung?.toLowerCase().contains(searchValue.toLowerCase()) == true) ?? false;
                            } else {
                              return false;
                            }
                          },
                                ...
                                Container(
                                  child: Column(
                                    children: [
                                      ...
                                      // Container(
                                      //   alignment: Alignment
                                      //       .centerLeft, // Căn sát lề trái
                                      //   padding: EdgeInsets.symmetric(
                                      //       horizontal:
                                      //           8), // Thêm padding nếu cần
                                      //   child: const Text(
                                      //     "Nơi đi:",
                                      //     style: TextStyle(
                                      //       fontFamily: 'Comfortaa',
                                      ...
                                      //                         item.maDiaDiem !=
                                      //                             null)
                                      //                     ? "${item.maDiaDiem} - ${item.tenDiaDiem}"
                                      //                     : "",
                                      //                 style: const TextStyle(
                                      //                   fontFamily: 'Comfortaa',
                                      //                   fontSize: 13,
                                      ...
                                      Container(
                                          ...
                                          child: DropdownButtonHideUnderline(
                                            child: DropdownButton2<String>(
                                              isExpanded: true,
                                              items: _diadiemList?.where((item) => (item.tenDiaDiem?.isNotEmpty == true))?.map((item) {
                                                return DropdownMenuItem<String>(
                                                  value: item.id,
                                                  child: Container(
                                                    child: Text(
                                                      (item.tenDiaDiem != null && item.maDiaDiem != null) ? "${item.maDiaDiem} - ${item.tenDiaDiem}" : "",
                                                      style: const TextStyle(
                                                        fontFamily: 'Comfortaa',
                                                        fontSize: 13,
                                                        fontWeight: FontWeight.w600,
                                              ...
                                              // value: DoiTacId,
                                              value: _diadiemList!.where((item) => (item.tenDiaDiem?.isNotEmpty == true && item.maDiaDiem?.isNotEmpty == true)).any((item) => item.id == NoiDenId) ? NoiDenId : null,
                                              onChanged: (newValue) {
                                                setState(() {
                                                  NoiDenId = newValue ?? "";
                                                });
                                              ...
                                              dropdownSearchData: DropdownSearchData(
                                                ...
                                                searchMatchFn: (item, searchValue) {
                                                  if (item is DropdownMenuItem<String>) {
                                                    ...
                                                    String itemId = item.value ?? "";
                                                    // Kiểm tra ID của item có tồn tại trong _vl.vitriList không
                                                    return _diadiemList?.any((viTri) => viTri.id == itemId && (viTri.tenDiaDiem?.toLowerCase().contains(searchValue.toLowerCase()) == true || viTri.maDiaDiem?.toLowerCase().contains(searchValue.toLowerCase()) == true)) ?? false;
                                                  } else {
                                                    return false;
                                                  }
                                                },
                                      ...
                                      Container(
                                          ...
                                          child: DropdownButtonHideUnderline(
                                            child: DropdownButton2<String>(
                                              isExpanded: true,
                                              items: _taixeList?.where((item) => (item.tenTaiXe?.isNotEmpty == true && item.maTaiXe?.isNotEmpty == true) && (item.id == TaiXeId || item.doiTac_Id == DoiTacId || item.doiTac_Id == "bc919654-7d86-4285-9e9a-6c0d37ddaaab"))?.map((item) {
                                                return DropdownMenuItem<String>(
                                                  value: item.id,
                                                  child: Container(
                                                    child: Text(
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              //       0.7, // Chiều rộng 70% màn hình
              //   child: ElevatedButton(
              //       onPressed: (_data?.soKhung != null &&
              //               _lido.text.isNotEmpty &&
              //               _lido.text != "Nhập lý do")
...
class Item extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (value == "" || value!.trim().isEmpty) {
      // Trả về một SizedBox với chiều cao = 0 nếu không có dữ liệu
