...
class CustomBodySuaChuaQLNew extends StatelessWidget {
  final String? id;
  CustomBodySuaChuaQLNew({required this.id});
  @override
  Widget build(BuildContext context) {
    return Container(
        child: BodySuaChuaQLNewScreen(
      id: id,
      lstFiles: [],
    ));
  }
...
class BodySuaChuaQLNewScreen extends StatefulWidget {
  final String? id;
  final List<CheckSheetFileModel?> lstFiles;
  const BodySuaChuaQLNewScreen({super.key, required this.lstFiles, required this.id});

  @override
  _BodySuaChuaQLNewScreenState createState() => _BodySuaChuaQLNewScreenState();
}

class _BodySuaChuaQLNewScreenState extends State<BodySuaChuaQLNewScreen> with TickerProviderStateMixin, ChangeNotifier {
  ...
  void _updateTongChiPhi(StateSetter dialogSetState) {
    double tong = 0.0;
    chiphiControllers.forEach((key, controller) {
  ...
  Future<void> getChiTietHM(String? phuongTien_Id, String? hangMuc_Id, StateSetter dialogSetState) async {
    ...
    _loading = true;
    try {
      final http.Response response = await requestHelper.getData('MMS_SuaChua/LichSuSuaChuaChiTiet?PhuongTien_Id=$phuongTien_Id&HangMuc_Id=$hangMuc_Id');
  ...
  Future<void> getAllHM(String? phuongTien_Id, StateSetter dialogSetState) async {
    _loading = true;
    try {
      final http.Response response = await requestHelper.getData('MMS_ThongTinTheoHangMuc/DSHangMucSuaChua?Id_PhuongTien=$phuongTien_Id');
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedData != null) {
          ...
          dialogSetState(() {
            _selectedItems = List.from(_hangmucListnew!.where((item) => item.isDenHanSC == true));
            _loading = false;
          });
        }
      } else {
  ...
  Future<void> postDataHuy(LichSuBaoDuongNewModel? scanData, String? liDo) async {
    ...
    try {
      ...
      final http.Response response = await requestHelper.postData('MMS_SuaChua/HuyYeuCauSuaChua?TenNhanVien=${_ub?.name}&MaNhanVien=${_ub?.maNhanVien}&User_Id=${_ub?.id}&LiDo=${liDo}', dataList.map((e) => e?.toJson()).toList());
  ...
  Future<void> postData(LichSuBaoDuongModel scanData, String? ngayDiBaoDuong, List<String> selectedIds) async {
    ...
    try {
      ...
      final http.Response response = await requestHelper.postData('MMS_SuaChua/XacNhanYeuCau?ids=${selectedIds.join("&ids=")}&NgayDiSuaChua=$ngayDiBaoDuong&TenNhanVien=${_ub?.name}&MaNhanVien=${_ub?.maNhanVien}&User_Id=${_ub?.id}', dataList.map((e) => e.toJson()).toList());
      print("statusCode: ${response.statusCode}");
      ...
      if (response.statusCode == 200) {
        ...
        body = "Bạn vừa được xác nhận 1 yêu cầu sửa chữa phương tiện";
      } else {
        String errorMessage = response.body.replaceAll('"', '');
        notifyListeners();
        _btnController.error();
  ...
  Future<void> postDataList(
    ...
    try {
      final http.Response response = await requestHelper.postData('MMS_SuaChua/XacNhanYeuCau?NgayDiSuaChua=$ngayDiBaoDuong&TenNhanVien=${_ub?.name}&MaNhanVien=${_ub?.maNhanVien}&User_Id=${_ub?.id}', dataList.map((item) => item.toJson()).toList());
      print("statusCode: ${response.statusCode}");
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
        print("data: ${decodedData}");
  ...
  Future<void> postDataFireBase(
    ...
    try {
      ...
      final http.Response response = await requestHelper.postData('MMS_Notification/PushThongBao_SuaChua?body=$body&listIds=$id&NguoiYeuCau=$nguoiYeuCau', newScanData?.toJson());
  ...
  void _showConfirmationDialogYeuCau(BuildContext context, String? bienSo1, int index) {
    ...
    selectedDate = _kehoachList?[index].ngayDiSuaChua;
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_hangmucListnew!.isNotEmpty)
                            Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: _hangmucListnew!.where((item) => item.isDenHanSC == true).map((item) {
                                ...
                                return CheckboxListTile(
                                  ...
                                  subtitle: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      buildRichText('Số KM hiện tại:', item?.soKM),
                                    ],
                                  ),
                                  value: isChecked,
  ...
  Future<void> postDataHoanThanh(LichSuBaoDuongModel? scanData, List<Map<String, dynamic>> danhSachChiPhi, int index, int? tongChiPhi, String? vatTu) async {
    _isLoading = true;
    try {
      var newScanData = scanData;
      newScanData?.bienSo1 = newScanData?.bienSo1 == 'null' ? null : newScanData?.bienSo1;
      ...
      final http.Response response = await requestHelper.postData('MMS_SuaChua/HoanThanhSuaChua?TenNhanVien=${_ub?.name}&MaNhanVien=${_ub?.maNhanVien}&User_Id=${_ub?.id}&TongChiPhi=$tongChiPhi&VatTuThayThe=$vatTu', requestBody);
      ...
      if (response.statusCode == 200) {
        ...
        _btnController.reset();
        await getListThayDoiKH(widget.id, soKhungController.text);
        body = "Bạn vừa được xác nhận 1 đề xuất hoàn thành sửa chữa phương tiện";
      } else {
        String errorMessage = response.body.replaceAll('"', '');
        notifyListeners();
        _btnController.error();
  ...
  _onSaveHoanThanh(int index, String? tongChiPhiSC, String? vatTu) async {
    setState(() {
      _loading = true;
    });
...
// Chuyển đổi danh sách URL thành chuỗi cách nhau bởi dấu phẩy
    ...
    int? tongChiPhi = int.parse(tongChiPhiSC!.replaceAll(RegExp(r'[.,]'), ""));
    final item = _kehoachList?[index];
    print("data kehoach = ${item?.id}");
    _data ??= LichSuBaoDuongModel();
    _data?.id = item?.id;
    ...
    AppService().checkInternet().then((hasInternet) {
      if (!hasInternet!) {
        ...
        postDataHoanThanh(_data!, danhSachChiPhi, index, tongChiPhi, vatTu).then((_) {
          setState(() {
            _IsTuChoi = false;
            _IsXacNhan = false;
            _textController.text = '';
  ...
  void _showConfirmationDialogHoanThanh(BuildContext context, int index, String? tongChiPhi, String? vatTu) {
    QuickAlert.show(
        context: context,
        ...
        onConfirmBtnTap: () {
          Navigator.of(context).pop();
          _onSaveHoanThanh(index, tongChiPhi, vatTu);
        });
  }

  void _showDetailsDialog2(BuildContext context, int index, String? ghiChu, String? soKM, String? tongChiPhi, String? vatTu) {
    final baoduongId = _kehoachList?[index].id;
    final image = _kehoachList?[index]?.hinhAnh;
    final TextEditingController _tongChiPhi = TextEditingController(text: tongChiPhi ?? "0");
                      ...
                      child: Column(
                        children: [
                          RoundedLoadingButton(
                            ...
                            controller: _btnController,
                            onPressed: () => _showConfirmationDialogHoanThanh(context, index, _tongChiPhi.text, _vatTu.text),
                          ),
                        ],
                      ),
                    ),
                                  ...
                                  return Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        buildRichText('Loại sửa chữa:', item?.loaiBaoDuong),
                                        buildRichText('Ghi chú:', item?.ghiChu),
                                        buildRichText('Chi phí:', item?.chiPhi),
                                        Divider(height: 1, color: Color(0xFFDDDDDD)),
                                      ],
              ...
              children: [
                TableRow(
                  children: [
                    ...
                    Container(
                      color: Colors.red,
                      child: _buildTableCell('Người đi sửa chữa', textColor: Colors.white),
                    ),
                    Container(
                      color: Colors.red,
                    ...
                    Container(
                      color: Colors.red,
                      child: _buildTableCell('Ngày đi sửa chữa', textColor: Colors.white),
                    ),
                    Container(
                      color: Colors.red,
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      InfoColumn(
                                        ...
                                        onDongYHoanThanh: () {
                                          _showDetailsDialog2(context, index, item?.ketQua, item?.soKM, item?.tongChiPhi, item?.vatTuThayThe); // Hành động ĐỒNG Ý
                                        },
                                        onTuChoi: () {
                                          _IsTuChoi = true;
...
class InfoColumn extends StatelessWidget {
  final String bienSo1, danhSachHangMuc; // Biển số 1
  final String ngay; // Thời gian yêu cầu
  final String ngayXacNhan, chiPhi; // Lý do đổi
  final String ngayDiSuaChua, tinhTrang, ngayDeXuatHoanThanh, nguoiDeXuatHoanThanh; // Nhà xe
  final String ngayHoanThanh, nguoiYeuCau, nguoiXacNhan, nguoiDiSuaChua, nguoiXacNhanHoanThanh, noiDung, ketQua, soKM, tenDiaDiem;
  final VoidCallback onDongY; // Hành động khi bấm ĐỒNG Ý
  final bool isSelected, isSuaChua, isDuyet, isYeuCau, isLenhHoanThanh, isHoanThanh; // Trạng thái tích chọn
