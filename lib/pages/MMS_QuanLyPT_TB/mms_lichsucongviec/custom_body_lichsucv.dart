...
import '../mms_ketquabaoduong/dsbaoduong.dart';
import '../mms_ketquabaoduong/lsxbaoduong.dart';
import '../mms_ketquadangkiem/dsdangkiem.dart';
import '../mms_ketquasuachua/ds_suachua.dart';

// ignore: use_key_in_widget_constructors
class CustomBodyLichSuCV extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(width: 100.w, child: BodyLichSuCVScreen());
  }
}

class BodyLichSuCVScreen extends StatefulWidget {
  const BodyLichSuCVScreen({Key? key}) : super(key: key);

  @override
  _BodyLichSuCVScreenState createState() => _BodyLichSuCVScreenState();
}

// ignore: use_key_in_widget_constructors, must_be_immutable
class _BodyLichSuCVScreenState extends State<BodyLichSuCVScreen> with TickerProviderStateMixin, ChangeNotifier {
  int currentPage = 0;
  int pageCount = 3;
  bool _loading = false;
        ...
        : SingleChildScrollView(
            child: Container(
              ...
              child: Wrap(
                ...
                children: [
                  if (userHasPermission(menuRoles, 'ket-qua-bao-duong-mobi'))
                    CustomButton(
                      'KẾT QUẢ BẢO DƯỠNG',
                      Stack(
                      ...
                      () {
                        _handleButtonTap(DSBaoDuongPage());
                      },
                    ),
                  if (userHasPermission(menuRoles, 'ket-qua-sua-chua-mobi'))
                    CustomButton(
                      'KẾT QUẢ SỬA CHỮA',
                      Stack(
                        alignment: Alignment.center,
                      ...
                      () {
                        _handleButtonTap(DSSuaChuaPage());
                      },
                    ),
                  if (userHasPermission(menuRoles, 'ket-qua-dang-kiem-mobi'))
                    CustomButton(
                      'KẾT QUẢ ĐĂNG KIỂM',
                      Stack(
                        alignment: Alignment.center,
                      ...
                      () {
                        _handleButtonTap(DSDangKiemPage());
                      },
                    ),
                ],
              ),
