...
class CustomBodyDanhSachThietBi extends StatelessWidget {
  CustomBodyDanhSachThietBi();
  @override
  Widget build(BuildContext context) {
    return Container(
        child: BodyDanhSachThietBiScreen(
      lstFiles: [],
    ));
  }
}

class BodyDanhSachThietBiScreen extends StatefulWidget {
  final List<CheckSheetFileModel?> lstFiles;
  const BodyDanhSachThietBiScreen({super.key, required this.lstFiles});

  @override
  _BodyDanhSachThietBiScreenState createState() => _BodyDanhSachThietBiScreenState();
}

class _BodyDanhSachThietBiScreenState extends State<BodyDanhSachThietBiScreen> with TickerProviderStateMixin, ChangeNotifier {
  ...
  void initState() {
    ...
    // getBienSo();
    Future.wait([getBienSoKhongQuanLy()]).then((results) {
      setState(() {
        _loading = false;
      });
    }).catchError((error) {
  ...
  Future<void> getBienSoKhongQuanLy() async {
    _loading = true;
    try {
      final http.Response response = await requestHelper.getData('MMS_DS_ThietBi/GetThietBiKhongQuanLy');
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
        print("datathietbi: ${decodedData}");
  ...
  Future<void> GetListPhuongTienKhongQuanLy(String? phuongTien_Id) async {
    _loading = true;
    try {
      final http.Response response = await requestHelper.getData('MMS_DS_ThietBi/GetListThietBiKhongQuanLy?ThietBi_Id=$phuongTien_Id');
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
                                                  ...
                                                  dropdownSearchData: DropdownSearchData(
                                                    ...
                                                    searchMatchFn: (item, searchValue) {
                                                      if (item is DropdownMenuItem<String>) {
                                                        ...
                                                        String itemId = item.value ?? "";
                                                        // Kiểm tra ID của item có tồn tại trong _vl.vitriList không
                                                        return _phuongtien?.any((baiXe) => baiXe.id == itemId && baiXe.maThietBi?.toLowerCase().contains(searchValue.toLowerCase()) == true) ?? false;
                                                      } else {
                                                        return false;
                                                      }
                                                    },
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            InfoRow(
              title: "Số giờ hoạt động:",
              contentYC: soKM_Adsun,
            ),
            const SizedBox(height: 4),
            InfoRow(
              title: "Số giờ ngày bảo dưỡng gần nhất:",
              contentYC: giaTri,
            ),
            // const SizedBox(height: 4),
            // CustomRichTextTT(
