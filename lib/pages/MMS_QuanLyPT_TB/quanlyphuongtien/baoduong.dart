...
class CustomBodyBaoDuong extends StatelessWidget {
  final String? id;
  CustomBodyBaoDuong({required this.id});
  @override
  Widget build(BuildContext context) {
    return Container(
        child: BodyBaoDuongScreen(
      id: id,
      lstFiles: [],
    ));
  }
...
class BodyBaoDuongScreen extends StatefulWidget {
  final String? id;
  final List<CheckSheetFileModel?> lstFiles;
  const BodyBaoDuongScreen({super.key, required this.id, required this.lstFiles});

  @override
  _BodyBaoDuongScreenState createState() => _BodyBaoDuongScreenState();
}

class _BodyBaoDuongScreenState extends State<BodyBaoDuongScreen> with TickerProviderStateMixin, ChangeNotifier {
  ...
  Future<void> postData(LichSuBaoDuongModel? scanData) async {
    ...
    try {
      ...
      final http.Response response = await requestHelper.postData('MMS_BaoCao/DiBaoDuong?TenNhanVien=${_ub?.name}&MaNhanVien=${_ub?.maNhanVien}&User_Id=${_ub?.id}', newScanData?.toJson());
      if (response.statusCode == 200) {
        var decodedData = jsonDecode(response.body);
...
class InfoColumn extends StatelessWidget {
  ...
  final String ngayXacNhan, ngayDeXuatHoanThanh, nguoiDeXuatHoanThanh, danhSachHangMuc; // Lý do đổi
  final String ngayDiBaoDuong, tinhTrang; // Nhà xe
  final String ngayHoanThanh, nguoiYeuCau, nguoiXacNhan, nguoiDiBaoDuong, nguoiXacNhanHoanThanh, noiDung, ketQua, chiPhi, tenDiaDiem;
  final VoidCallback onDongY; // Hành động khi bấm ĐỒNG Ý
  final bool isSelected, isBaoDuong, isDuyet, isLenhHoanThanh, isHoanThanh, isYeuCau; // Trạng thái tích chọn
                  ...
                  Padding(
                    ...
                    child: Container(
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: onDongY, // Hành động ĐỒNG Ý
                        child: const Text("ĐỀ XUẤT HOÀN THÀNH BẢO DƯỠNG"),
                      ),
                    ),
                  ),
                ],
