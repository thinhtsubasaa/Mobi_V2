...
class CustomBodyYeuCauSuaChua extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(
        child: BodyYeuCauSuaChuaScreen(
      lstFiles: [],
    ));
  }
}

class BodyYeuCauSuaChuaScreen extends StatefulWidget {
  final List<CheckSheetFileModel?> lstFiles;
  const BodyYeuCauSuaChuaScreen({super.key, required this.lstFiles});

  @override
  _BodyYeuCauSuaChuaScreenState createState() => _BodyYeuCauSuaChuaScreenState();
}

class _BodyYeuCauSuaChuaScreenState extends State<BodyYeuCauSuaChuaScreen> with TickerProviderStateMixin, ChangeNotifier {
    ...
    Geolocator.getCurrentPosition(
      ...
      AppService().checkInternet().then((hasInternet) {
        if (!hasInternet!) {
          ...
          postData( "", _data?.toaDo ?? "", _data?.soKhung ?? "", _ghiChu.text, _data?.hinhAnh ?? "").then((_) {
            setState(() {
              _data = null;
              barcodeScanResult = null;
              _ghiChu.text = '';
