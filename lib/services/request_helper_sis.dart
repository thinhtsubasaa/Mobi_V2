...
class RequestHelperSIS {
  ...
  _getInfo() async {
    ...
    API = 'https://apisis.thilogi.vn/api/';
  }
  ...
  loginAction(data) async {
    ...
    return await http.post(
      Uri.parse('https://apisis.thilogi.vn/token'),
      headers: headers,
      body: data,
    );
  }
  ...
  refreshTokenAction(tokenInfo) async {
    try {
      ...
      http.Response request = await http.post(
        Uri.parse('https://apisis.thilogi.vn/token/refresh'),
  ...
  uploadFile(File file) async {
    ...
    try {
      SharedPreferences sp = await SharedPreferences.getInstance();
      http.MultipartRequest request = http.MultipartRequest("POST", Uri.parse('${sp.getString('apiUrl2')}/api/MMS_Upload/UpLoadLichSu'));
      request.headers.addAll(_setHeaders());
  ...
  uploadAvatar(File file) async {
    ...
    try {
      SharedPreferences sp = await SharedPreferences.getInstance();
      http.MultipartRequest request = http.MultipartRequest("POST", Uri.parse('${sp.getString('apiUrl2')}/api/upload/UpLoadAvatar'));
      request.headers.addAll(_setHeaders());
      http.MultipartFile multipartFile = await http.MultipartFile.fromPath('file', file.path);
      request.files.add(multipartFile);
      var streamedResponse = await request.send();
